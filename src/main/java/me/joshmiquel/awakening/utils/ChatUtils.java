package me.joshmiquel.awakening.utils;
...
public class ChatUtils {
    ...
    public static void sendCenteredMessage(Player player, String message) {
        ...
        else message = ChatColor.translateAlternateColorCodes('&', message);

        int messagePxSize = 0;
        boolean previousCode = false;
        boolean isBold = false;

        for (char c : Objects.requireNonNull(message).toCharArray()) {
            if (c == '§') {
                previousCode = true;
    ...
    public static String replaceUnderscoreAndCapitalize(String string) {
        String removeUnderscore = string.replaceAll("_", " ");
        String capitalize = WordUtils.capitalize(removeUnderscore);
